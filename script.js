...
// Stufe 9 und 10 gleichermaßen: Mathe, Deutsch, Englisch,
// WPF Technik, WPF Französisch, WPF AES.
function examSubjectAllowed(fach, level) {
    if (level !== 9 && level !== 10) return false;
    const f = (fach || '').toLowerCase();
    return f.includes('mathe') || f.includes('deutsch') || f.includes('englisch') ||
           f.includes('technik') || f.includes('franz') || f.includes('aes');
}
...
function openExamCategoryConfig(lnw) {
    ...
    // Hilfsfunktion: Kategorien-Liste auf 100 % normieren, sonst Standard nehmen
    const normSubs = (list, fallback) => {
        if (!list.length) return fallback;
        const sum = list.reduce((s, c) => s + (c.subWeight ?? c.weight ?? 0), 0) || 1;
        const subs = list.map(c => ({ id: c.id, name: c.name, type: c.type || 'schriftlich', weight: Math.round((c.subWeight ?? c.weight ?? 0) / sum * 100) }));
        const s2 = subs.reduce((s, c) => s + c.weight, 0);
        subs[subs.length - 1].weight += 100 - s2;
        return subs;
    };
    ...
    if (!split) {
        ...
        const cur = getGradeCategories(lnw);
        split = {
            pName: 'Prüfungsleistung', pWeight: 50,
            jName: 'Jahresleistung', jWeight: 50,
            pSubs: normSubs(cur.filter(c => c.group === 'pruefung'),
                [{ id: 'cat_' + Date.now() + '_pr', name: 'Prüfung', weight: 100, type: 'schriftlich' }]),
            subs: normSubs(cur.filter(c => c.group !== 'pruefung'),
                [{ id: 'cat_' + Date.now() + '_js', name: 'Schriftlich', weight: 50, type: 'schriftlich' },
                 { id: 'cat_' + (Date.now() + 1) + '_jm', name: 'Mündlich', weight: 50, type: 'muendlich' }])
        };
    }
    // Migration: ältere Stände hatten EINE ungeteilte Prüfungs-Kategorie (pId)
    if (!split.pSubs || !split.pSubs.length) {
        split.pSubs = [{ id: split.pId || ('cat_' + Date.now() + '_pr'), name: 'Prüfung', weight: 100, type: 'schriftlich' }];
    }
    ...
    document.getElementById('exam-psub-list').innerHTML = split.pSubs.map(s => catRowHtml(s)).join('');
...
function ensureSwitchButton(page) {
    ...
    if (!row) return;
    if (!row.querySelector('.switch-to-noten-btn')) {
    ...
    if (!row.querySelector('.comp-overview-btn')) {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'app-btn comp-overview-btn';
        btn.style.cssText = 'font-size:12px; padding:6px 10px;';
        btn.title = 'Alle Kompetenzen dieses Schülers über alle Lernnachweise – sortiert nach erreicht / teilweise / nicht erreicht';
        btn.textContent = '📈 Kompetenz-Übersicht';
        row.insertBefore(btn, row.querySelector('.switch-to-noten-btn'));
    }
}
...
function canonicalizeLnwBar(bar) {
    ...
    const catItem = lnwMakeItem('grade-categories-btn', '⚙️ Notengewichtung', 'Notengewichtung für dieses Fach festlegen');
...
function renderGradePanel(panel, page, lnwWrapper) {
    ...
    let html = `<div class="grade-panel-header">
        ...
        <div class="grade-panel-actions">
            ...
            ${!allNoten ? `<button class="app-btn nb-config-btn" data-student="${page.id}" title="Notengewichtung für dieses Fach festlegen">⚙️ Notengewichtung</button>` : ''}
            <button class="app-btn primary-btn nb-switch-rb" data-student="${page.id}">📋 Zur Rückmeldung</button>
        </div>
    </div>`;

    html += `<div class="grade-cats-where">Die Notengewichtung gilt für das ganze Fach – änderbar über ${allNoten ? '„⚙️ Mehr → Notengewichtung" in der Klassenleiste' : 'den Button „⚙️ Notengewichtung" oben rechts'}.</div>`;
...
function openGradeConfig(lnwWrapper) {
    ...
    if (titleEl) titleEl.textContent = fach ? `Notengewichtung · ${fach}` : 'Notengewichtung';
    const cats = getGradeCategories(lnwWrapper);
...
function catRowHtml(cat) {
    ...
    const std = ['schriftlich', 'muendlich', 'praktisch'];
    // Eigene (selbst erstellte) Noten-Arten werden als eigener Eintrag angezeigt
    const customOpt = (!std.includes(type) && type !== '__custom')
        ? `<option value="${escapeHtml(type)}" selected>🏷️ ${escapeHtml(type)}</option>` : '';
    return `<div class="nb-cat-row">
        ...
        <select class="nb-type-inp" title="Art der Note">
            ...
            ${customOpt}
            <option value="__custom">✏️ Eigene Art…</option>
        </select>
...
// "✏️ Eigene Art…" gewählt -> Name abfragen und als neue Noten-Art übernehmen
document.addEventListener('change', (e) => {
    if (!e.target.classList.contains('nb-type-inp') || e.target.value !== '__custom') return;
    const name = prompt('Name der eigenen Noten-Art (z. B. Referat, Heftführung, Projekt):');
    const v = (name || '').trim();
    if (v) {
        let opt = Array.from(e.target.options).find(o => o.value === v);
        if (!opt) {
...
// ==========================================
// NOTENÜBERSICHT ALS DRUCKSEITE
// ==========================================
// Erzeugt eine unsichtbare, druckfertige Notenübersicht für einen Schüler.
// Wird von printPagesSmart (index.html) genutzt und danach wieder entfernt.
function buildGradePrintPage(page, lnw) {
    if (!page || !lnw) return null;
    ...
    const klasse = cls?.getAttribute('data-klasse') || '';
    const total  = calcTotal(grades, cats);
    const today  = fmtDateDE(new Date().toISOString().split('T')[0]);

    let inner = `<div style="font-family:Arial, Helvetica, sans-serif; font-weight:normal; color:#000;">
        <div style="display:flex; justify-content:space-between; align-items:baseline; border-bottom:2px solid #000; padding-bottom:6px;">
            <span style="font-size:20px; font-weight:bold;">Notenübersicht · ${escapeHtml(name)}</span>
            <span style="font-size:12px;">${escapeHtml(fach)} · Klasse ${escapeHtml(klasse)} · Stand: ${today}</span>
        </div>`;

    cats.forEach(cat => {
        const cGrades = grades.filter(g => g.catId === cat.id);
        const cAvg    = calcCatAvg(grades, cat.id);
        const label   = (cat.groupLabel && cat.groupLabel !== cat.name ? cat.groupLabel + ' · ' : '') + cat.name;
        inner += `<div style="margin-top:14px; font-size:14px; font-weight:bold;">${escapeHtml(label)}
            <span style="font-weight:normal; font-size:11px;">(Gewichtung ${cat.weight} %)</span></div>
        <table style="width:100%; border-collapse:collapse; margin-top:4px; font-size:12px;">
            <thead><tr>
                <th style="background:#e8eef5; text-align:left;">Bezeichnung</th>
                <th style="background:#e8eef5; width:90px;">Datum</th>
                <th style="background:#e8eef5; width:60px;">Note</th>
            </tr></thead><tbody>`;
        if (cGrades.length) {
            cGrades.forEach(g => {
                inner += `<tr>
                    <td style="text-align:left;">${escapeHtml(g.name)}</td>
                    <td style="text-align:center;">${escapeHtml(fmtDateDE(g.date))}</td>
                    <td style="text-align:center; font-weight:bold;">${fmtGradeValue(g.value)}</td></tr>`;
            });
            inner += `<tr>
                <td colspan="2" style="text-align:right; font-weight:bold;">Durchschnitt</td>
                <td style="text-align:center; font-weight:bold;">${fmtNote(cAvg)}</td></tr>`;
        } else {
            inner += `<tr><td colspan="3" style="text-align:center; color:#666;">Keine Einträge</td></tr>`;
        }
        inner += `</tbody></table>`;
    });

    inner += `<div style="margin-top:18px; border:2px solid #000; padding:10px 14px; display:flex; justify-content:space-between; font-size:16px; font-weight:bold;">
        <span>Gesamtnote</span><span>${fmtNote(total)}</span>
    </div>
    <input class="student-name-input" value="${escapeHtml(name)}" style="display:none;">
    </div>`;

    const wrap = document.createElement('div');
    wrap.className = 'page grade-print-page';
    wrap.id = 'gradeprint_' + page.id;
...
// ==========================================
// KOMPETENZ-ÜBERSICHT EINES SCHÜLERS
// (alle LNWs eines Fachs oder alle Fächer, sortiert nach −/o/+)
// ==========================================
let _compOverviewPageId = null;

// Sammelt alle Bögen desselben Schülers – im Fach (über die stabile Schüler-ID)
// oder über alle Fächer (über den Namen).
function collectStudentPagesForScope(page, scope) {
    const list = [];
    if (scope === 'all') {
        const key = studentKey(getStudentNameFromPage(page));
        if (!key) return list;
        document.querySelectorAll('.subject-wrapper').forEach(subject => {
            ...
            subject.querySelectorAll('.lnw-wrapper').forEach(lnw => {
                lnw.querySelectorAll('.workspace .page').forEach(p => {
                    if (studentKey(getStudentNameFromPage(p)) === key) {
                        list.push({ page: p, lnwNr: lnw.getAttribute('data-lnw-nr') || '', fach });
                    }
                });
            });
        });
        ...
        cls?.querySelectorAll('.lnw-wrapper').forEach(lnw => {
            lnw.querySelectorAll('.workspace .page').forEach(p => {
                const match = uid ? (p.dataset.studentUid === uid) : (studentKey(getStudentNameFromPage(p)) === key);
                if (match) list.push({ page: p, lnwNr: lnw.getAttribute('data-lnw-nr') || '', fach });
            });
        });
    }
    return list;
...
function renderCompOverview(scope) {
    const page = document.getElementById(_compOverviewPageId);
    if (!page) return;
    const name = getStudentNameFromPage(page);
    const fach = page.closest('.subject-wrapper')?.getAttribute('data-fach') || '';
    document.getElementById('comp-overview-title').textContent = `📈 Kompetenz-Übersicht · ${name}`;
    document.getElementById('comp-overview-sub').textContent = scope === 'all' ? 'Alle Fächer · alle Lernnachweise' : `${fach} · alle Lernnachweise dieses Fachs`;
    document.getElementById('comp-scope-fach').classList.toggle('primary-btn', scope !== 'all');
    document.getElementById('comp-scope-all').classList.toggle('primary-btn', scope === 'all');

    const buckets = { nicht: [], teils: [], erreicht: [] };
    let unrated = 0;
    collectStudentPagesForScope(page, scope).forEach(({ page: p, lnwNr, fach: f }) => {
        p.querySelectorAll('tr[data-row-id]').forEach(tr => {
            const text = (tr.querySelector('.kompetenz-text')?.innerText || '').replace(/…/g, '').trim();
            if (!text) return;
            const niveau = tr.closest('.niveau-section')?.getAttribute('data-section') || '';
            const bx = tr.querySelector('.x-marker-black');
            if (!bx || bx.classList.contains('inactive') || !bx.style.left) { unrated++; return; }
            const pct = parseFloat(bx.style.left) / 100;
            const entry = { text, lnwNr, fach: f, niveau };
            if (pct < 1 / 3) buckets.nicht.push(entry);
            else if (pct < 2 / 3) buckets.teils.push(entry);
            else buckets.erreicht.push(entry);
        });
    });

    const row = (e) => `<div class="comp-row">
        ${e.niveau ? `<span class="comp-niv niv-${e.niveau.toLowerCase()}">${e.niveau}</span>` : ''}
        <span class="comp-text">${escapeHtml(e.text)}</span>
        <span class="comp-meta">${scope === 'all' ? escapeHtml(e.fach) + ' · ' : ''}LNW ${escapeHtml(e.lnwNr)}</span>
    </div>`;
    const bucket = (cls, title, arr) => `<div class="comp-bucket ${cls}">
        <h4>${title} <span class="comp-count">${arr.length}</span></h4>
        ${arr.length ? arr.map(row).join('') : '<div class="comp-empty">Keine Kompetenzen in dieser Gruppe.</div>'}
    </div>`;

    let html = bucket('b-nicht', '− Nicht erreicht', buckets.nicht)
             + bucket('b-teils', 'o Teilweise erreicht', buckets.teils)
             + bucket('b-erreicht', '+ Erreicht', buckets.erreicht);
    if (unrated) html += `<div class="comp-unrated">${unrated} Kompetenz${unrated === 1 ? '' : 'en'} ohne Bewertung (kein X gesetzt).</div>`;
    document.getElementById('comp-overview-body').innerHTML = html;
}
...
document.addEventListener('click', function(e) {
    ...
    if (e.target.id === 'exam-sub-add-btn' || e.target.id === 'exam-psub-add-btn') {
        const list = document.getElementById(e.target.id === 'exam-psub-add-btn' ? 'exam-psub-list' : 'exam-sub-list');
        if (!list) return;
        const id = 'cat_' + Date.now() + '_' + Math.random().toString(36).slice(2, 5);
        list.insertAdjacentHTML('beforeend', catRowHtml({ id, name: 'Neue Kategorie', weight: 0, type: 'schriftlich' }));
        equalizeCatWeights(list);
    ...
    if (e.target.id === 'hotspots-info-btn') {
        const t = document.getElementById('hotspots-info-text');
        if (t) t.style.display = (t.style.display === 'none' || !t.style.display) ? 'block' : 'none';
        return;
    }

    // Kompetenz-Übersicht eines Schülers öffnen / Umfang umschalten
    if (e.target.classList.contains('comp-overview-btn')) {
        const page = e.target.closest('.page');
        if (page) {
            _compOverviewPageId = page.id;
            renderCompOverview('fach');
            document.getElementById('comp-overview-modal').style.display = 'flex';
        }
        return;
    }

    if (e.target.id === 'comp-scope-fach' || e.target.id === 'comp-scope-all') {
        renderCompOverview(e.target.id === 'comp-scope-all' ? 'all' : 'fach');
        return;
    }
});
...
document.addEventListener('change', (e) => {
    ...
    // Im Rückmeldebogen direkt das passende Niveau-Tab aktivieren
    if (typeof window.setPageNiveauTab === 'function') window.setPageNiveauTab(page, val);
    document.querySelectorAll('.niveau-popover').forEach(p => p.remove());
    if (typeof window.saveAppLocal === 'function') window.saveAppLocal();
});
...
document.getElementById('exam-config-save')?.addEventListener('click', () => {
    ...
    const readSubs = (listId) => Array.from(document.querySelectorAll('#' + listId + ' .nb-cat-row')).map(row => ({
        id:     row.querySelector('.nb-name-inp').getAttribute('data-cat-id'),
        name:   row.querySelector('.nb-name-inp').value.trim() || 'Kategorie',
        weight: Math.round(parseFloat(row.querySelector('.nb-weight-inp').value) || 0),
        type:   row.querySelector('.nb-type-inp')?.value || 'schriftlich'
    }));
    const pSubs = readSubs('exam-psub-list');
    const subs  = readSubs('exam-sub-list');

    if (!pSubs.length) { alert('Die Prüfungsleistung braucht mindestens eine Kategorie.'); return; }
    if (!subs.length) { alert('Die Jahresleistung braucht mindestens eine Kategorie.'); return; }
    if (pSubs.reduce((s, c) => s + c.weight, 0) !== 100) {
        alert('Die Kategorien der Prüfungsleistung müssen zusammen 100 % ergeben.');
        return;
    }
    ...
    const flat = [];
    const pushGroup = (list, gW, group, label) => {
        let acc = 0;
        list.forEach((s, i) => {
            let w = (i === list.length - 1) ? (gW - acc) : Math.round(gW * s.weight / 100);
            if (w < 0) w = 0;
            acc += w;
            flat.push({ id: s.id, name: s.name, weight: w, type: s.type, group, groupLabel: label, subWeight: s.weight });
        });
    };
    pushGroup(pSubs, pW, 'pruefung', pName);
    pushGroup(subs, jW, 'jahr', jName);

    subject.setAttribute('data-grade-categories', JSON.stringify(flat));
    subject.setAttribute('data-exam-split', JSON.stringify({ pName, pWeight: pW, jName, jWeight: jW, pSubs, subs }));
